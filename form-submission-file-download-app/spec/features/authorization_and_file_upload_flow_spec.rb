...
feature 'Visitor logs in' do
  ...
  scenario 'without PROTECTED_FILE_LINK_PROPERTY' do
    HUBSPOT_CLIENT_ID = ENV['HUBSPOT_CLIENT_ID']
    HUBSPOT_CLIENT_SECRET = ENV['HUBSPOT_CLIENT_SECRET']
    allow(ENV).to receive(:[])
    allow(ENV).to receive(:[]).with('HUBSPOT_CLIENT_ID').and_return(HUBSPOT_CLIENT_ID)
    allow(ENV).to receive(:[]).with('HUBSPOT_CLIENT_SECRET').and_return(HUBSPOT_CLIENT_SECRET)
    allow(ENV).to receive(:[]).with('PROTECTED_FILE_LINK_PROPERTY').and_return(nil)

    visit root_path
    expect(page).to have_content('Please specify PROTECTED_FILE_LINK_PROPERTY in .env')
  end
  ...
  scenario 'clicking on OAuth2' do
    ...
    using_wait_time 10 do
      ...
      expect(page).to have_css('.form-create')
    end
  end
end
